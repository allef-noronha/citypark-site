...
// A função 'normaliza' remove acentos, converte para letras minúsculas, 
// remove espaços extras e retorna uma string limpa.
const normaliza = (s) =>
  String(s || "")
    .toLowerCase()
    .normalize("NFD")
    ...
    return rows
      ...
      .map((it) => {
        ...
        const condition = paymentPlan.format({
          price: item.preco,
          downPayment: item.sinal,
          monthlyInstallment: item.parcela,
          semiannualInstallment: item.intercalada,
          keys: item.chaves
        });

        return {
          ...item,
          preco: condition.price,
          sinal: condition.downPayment,
          parcela: condition.monthlyInstallment,
          intercalada: condition.semiannualInstallment,
          chaves: condition.keys
        };
      })
...
function mostrarDetalhes(index) {
  ...
  // Função que esconde os valores se o status incluir "reservad" ou "vendid", cobrindo variações como "reservadO" e "vendidA" + variações.
